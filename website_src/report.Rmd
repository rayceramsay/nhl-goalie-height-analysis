...
plot = goalie_counts_by_country_and_season |>
  filter(seasonStartYear >= 2005) |>
  mutate(birthCountryCode = reorder(birthCountryCode, -n)) |>
  ggplot(aes(x = seasonStartYear, y = n, fill = birthCountryCode)) +
...
plot = established_goalies |>
  ...
  cor() |>
  ggcorrplot(lab = T, title = "Correlation Between Height, Time, and Performance Metrics\nof Established NHL Goalies Since 2000")
...
plot = goalie_heights_per_season |>
  rename(meanHeight = mean_height) |>
  ggplot(aes(x = seasonStartYear, y = meanHeight)) +
...
plot = goalie_heights_per_season |>
  mutate(minHeight = min_height) |>
  ggplot(aes(x = seasonStartYear, y = minHeight)) +
  geom_line(color="lightblue") +
  geom_point(color="#000099") +
  theme_minimal() +
  labs(
...
plot = goalie_heights_per_season_country |>
  rename(meanHeight = mean_height) |>
  ggplot(aes(x = seasonStartYear, y = meanHeight, colour = birthCountryCode)) +
  geom_line() +
  geom_point() +
...
plot = prospect_heights_per_season_and_nation |>
  rename(meanHeight = mean_height) |>
  ggplot(aes(x = draftYear, y = meanHeight, colour = nation)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  labs(
...
plot = league_stats_by_season |>
  rename(meanSavePct = mean_savePct) |>
  ggplot(aes(x = seasonStartYear, y = meanSavePct)) +
  geom_line(color="lightblue") +
  geom_point(color="#000099") +
  theme_minimal() +
  labs(
...
plot = league_stats_by_season |>
  rename(meanGAA = mean_goalsAgainstAverage) |>
  ggplot(aes(x = seasonStartYear, y = meanGAA)) +
  geom_line(color="lightblue") +
  geom_point(color="#000099") +
  theme_minimal() +
  labs(
...
plot = established_league_stats_by_season_undersized |>
  rename(meanSavePct = mean_savePct) |>
  ggplot(aes(x = seasonStartYear, y = meanSavePct, color = undersized)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  labs(
...
plot = established_league_stats_by_season_undersized |>
  rename(meanGAA = mean_goalsAgainstAverage) |>
  ggplot(aes(x = seasonStartYear, y = meanGAA, color = undersized)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  labs(
...
plot = imp_df[order(imp_df$Importance, decreasing = TRUE), ] |>
...
plot = partial(gaa_rf, pred.var = "Height", plot = FALSE) |>
...
# Extract the effect of height in the SV% RF
plot = partial(savePct_rf, pred.var = "Height", plot = FALSE) |>
  ggplot(aes(x = Height, y = yhat)) +
